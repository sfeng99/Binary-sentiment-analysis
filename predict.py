...
class SentimentAnalyzer:
    def __init__(self, path='/tmp/SST-2', model_type='albert-xxlarge-v2'):
        self.path = path
        self.model_type = model_type
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_type, do_lower_case=True)
...
if __name__ ==  '__main__':
    with open('glue_data/SST-2/test.txt', 'r') as f:
        text = f.readline()
        analyzer = SentimentAnalyzer()
        res = []
        while text:
            t = analyzer.predict(text)['logits'][1]
            res.append(t)
            text = f.readline()
